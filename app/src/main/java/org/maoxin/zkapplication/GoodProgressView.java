...
{
    private int[] mColors = { Color.GREEN, Color.RED};//进度条颜色（渐变色的2个点）
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            switch (attr)
            {
                case R.styleable.GoodProgressView_startColor:
                    // 渐变色之起始颜色，默认设置为红色
                    mColors[0] = a.getColor(attr,Color.GREEN);
                    break;
                case R.styleable.GoodProgressView_endColor:
                    // 渐变色之结束颜色，默认设置为品红
                    mColors[1] = a.getColor(attr, Color.RED);
                    break;
...
// float width = 300*unit;//绘画区域的长度
        ...
        //float[] position = {0, (float)0.33, (float)0.66,1};
        int[] colors ={0xFF1E90FF,0xFF00CED1,0xFF90FD90,0xFFFFD700,0xFFFF4500};
        float[] position = {(float)0.1,(float)0.3,(float)0.5,(float)0.7,(float)0.9};
        //设置渐变色区域
        LinearGradient shader = new LinearGradient(0, 0, offsetWidth*2+progressWidth , 0, colors, null,
                Shader.TileMode.CLAMP);
        // LinearGradient shader = new LinearGradient(0, 0, getMeasuredWidth()+1-1 , 0, colors, position,
                //         Shader.TileMode.CLAMP);
